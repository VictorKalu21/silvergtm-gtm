...
// keep each run to <= 6 batches of 20 leads and run ONE sweep at a time: the budget is shared by concurrent runs.
...
Then read \`${RUN}/owner/${DIR}/batches/batch-${i}-in.json\`: an array of leads with business_name, city, state, zip, website, brand_family, a ready-made query, and a registry domain. Before anything else, load the search tool: call ToolSearch with query "select:WebSearch" (it is a deferred tool and is not callable until loaded). Then, for EACH lead:
...
Write ONE JSON object to \`${RUN}/owner/${DIR}/batches/batch-${i}-out.json\` keyed by place_id, each value shaped:
