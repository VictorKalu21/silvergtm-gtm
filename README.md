...
| [05](processes/05-shopify-brands-not-on-amazon.md) | Shopify brands NOT on Amazon | You need established US physical-product Shopify/DTC brands with no official Amazon.com presence (or the inverse), screened from a free technographic census (HTTP Archive / Tranco×DNS), gated on the store's own `/meta.json` + `/products.json`, Amazon presence verified by autocomplete + rendered search. |
