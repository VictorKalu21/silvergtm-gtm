...
const IN=arg('in'), RS=arg('runsheet'), OUT=arg('out');
...
const OH=[...H,'city_source']; let n=0;
const out=rows.map(r=>{
  ...
  if(!(r[iC]||'').trim() && isFinite(la) && isFinite(lo)){
    ...
    if(best){r=[...r]; r[iC]=best.label; n++; return [...r,'nearest_tile'];}
  }
  return [...r,'google'];
});
fs.writeFileSync(OUT,[OH.map(esc).join(','),...out.map(r=>r.map(esc).join(','))].join('\n')+'\n');
console.log(`backfilled city on ${n}/${rows.length} address-less SABs from the nearest runsheet tile (city_source=nearest_tile)`);
