...
`assemble_deliverable.py` writes `deliverable/emails_final_unverified.csv` in the shape
...
address, and writes `deliverable/emails_final.csv` — the file to hand to:
    ...
    node skills/google-maps-scrape/build-plusvibe.js base \
      --leads <run>/deliverable/atlas_uk_foundation_repair_maps_qualified.csv \
