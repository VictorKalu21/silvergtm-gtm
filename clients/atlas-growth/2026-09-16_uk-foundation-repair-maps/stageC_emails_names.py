...
Ported from clients/atlas-growth/2026-09-16_uk-foundation-repair/stageC_emails_names.py for the
2026-09-16 UK MAPS run. The LOGIC is unchanged — same ranking, same regexes, same buckets. Only
the column/join shapes moved:

  * `domain` -> `root_domain` (this run's collapse-domains.js spine names it that).
  * site text is joined through `rep_place_id`: collapse-domains.js paid for ONE fetch per root
    domain, so a branch row reads its representative's pages and emails. `emails_from_rep` records
    when that happened. Same domain, same mailbox — but it is recorded rather than assumed.
  * **THIS SCRAPE HAS NO MAPS EMAIL COLUMN.** The export run carried `email_maps` (57% populated);
    a live scraper.tech pull does not return an email field at all, so leads_clean.csv and
    everything downstream of it has none. Every Maps-vs-site comparison column is therefore
    still emitted (the deliverable's shape must not change between runs) but degrades to blank /
    `no_maps_email`, and the script prints a one-line notice saying so. If a future run does carry
    the column, it is picked up automatically — nothing else needs changing.
  * reads owner/site_text.jsonl in the fetch-sites shape: `pages[]` ({url,label,text}) + `emails[]`
    + a flattened `text`. Emails come from the `emails[]` array as before.

WHAT THIS SCRIPT IS NOT: it does not name the decision-maker. The regex name extraction below
produces HINTS ONLY (`site_people_keep`, `first_name`, `name_confidence`) for QA and for matching a
person-shaped mailbox to a contact. The MODEL read (owner-prompt.md via prep-owner-batches.js)
is the only thing allowed to decide who the owner is — SKILL STEP 6, and the whole point of
owner-finding.md's "let a small model read clean text and name the people".

email ranking: person-shaped local part (first.last / f.last) > named generic
(info/enquiries/hello/sales/office/admin/contact/mail/enquiry) > other. Own-domain preferred over
third-party; a third-party domain scraped off the site is rejected outright; free-mail (gmail etc.)
is kept only when it is the Maps-listed email or was found on the business's OWN site.

Usage:  python3 stageC_emails_names.py [leads_qualified.csv]   ->  leads_qualified_contacts.csv
"""
import csv, json, re, collections, os, sys

HERE = os.path.dirname(os.path.abspath(__file__))
IN = sys.argv[1] if len(sys.argv) > 1 else os.path.join(HERE, 'leads_qualified.csv')
...
HAS_MAPS_EMAIL = bool(rows) and 'email_maps' in rows[0]
...
for r in rows:
    ...
    s=site.get(rep) or {}
    from_rep = bool(s) and rep != r['place_id']
    txt=s.get('text') or ''
    dom=r.get('root_domain') or ''
    cands=[]
    maps_e=(r.get('email_maps') or '').strip().lower()
    if maps_e: cands.append((maps_e,'maps'))
    ...
    r2['emails_from_rep']='Y' if from_rep else ''
    ...
    if not HAS_MAPS_EMAIL:
        r2['email_maps']=''
        r2['maps_email_on_site']='no_maps_email'
        ...
        if best:
            r2['email_source']=('both' if (best[1]==maps_e and best[1] in site_set) else 'maps' if best[1]==maps_e else 'site')
        else: r2['email_source']=''
    ...
    # role-pattern names from site text  (HINTS ONLY — the model read names people)
    found=[]
    ...
    if not fn and kept:
        ...
        parts=nm.split(); fn,ln,conf,ev=parts[0],' '.join(parts[1:]),'site_role_pattern:'+role,evd; r2['role_bucket_hint']=b
    elif fn and kept and kept[0][0].lower().split()[-1]==ln.lower().split()[-1]:
        r2['role_bucket_hint']=kept[0][3]; conf=conf+'+role:'+kept[0][1]
    r2['first_name_hint']=fn; r2['last_name_hint']=ln; r2['name_confidence']=conf; r2['name_evidence']=ev[:300]
    stats['email']+=bool(r2['email']); stats['person_email']+=(r2['email_type']=='person'); stats['name_hint']+=bool(fn); stats['name_hint_confirmed']+=(conf=='email+site')
    out.append(r2)
...
if not HAS_MAPS_EMAIL:
    print('NOTE: this scrape has NO email_maps column (a live scraper.tech pull returns no email field).')
    print('      Every email below came from the on-site harvest; the Maps-vs-site comparison columns are')
    print('      emitted for shape compatibility and read no_maps_email / new_from_site.')
print(len(out),"rows ->",os.path.basename(OUT),dict(stats))
print('email types:',collections.Counter(r['email_type'] for r in out).most_common())
print('emails inherited from a collapse representative:',sum(1 for r in out if r['emails_from_rep']))
print('NAME COLUMNS ARE HINTS ONLY — the owner-prompt model read is what names the decision-maker.')
