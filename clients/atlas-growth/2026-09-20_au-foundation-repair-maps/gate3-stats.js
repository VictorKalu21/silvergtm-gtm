...
// ---- Australian state from the state token ----------------------------------------------------
const ST_RE = /\b(NSW|VIC|QLD|SA|WA|TAS|ACT|NT)\b/;
const CENTROID = [-32.2054, 136.1074]; // Google's no-location placeholder (GATE1 §7.7)
function nationOf(r) {
  const m = String(r.city || '').match(ST_RE) || String(r.full_address || '').match(ST_RE);
  if (m) return { nation: m[1], area: m[1] };
  const la = parseFloat(r.latitude), ln = parseFloat(r.longitude);
  if (isFinite(la) && Math.abs(la - CENTROID[0]) < 0.01 && Math.abs(ln - CENTROID[1]) < 0.01) return { nation: 'UNKNOWN (centroid placeholder, no address)', area: '' };
  return { nation: 'UNKNOWN (no state token)', area: '' };
}
const SPLIT = new Set();
