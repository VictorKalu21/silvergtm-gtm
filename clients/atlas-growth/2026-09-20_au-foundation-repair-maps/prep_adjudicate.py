"""Build adjudication batches: services-preferred text slice (~2.5k chars) + evidence, 25 leads per batch.  RUN-FOLDER ONE-OFF.

Ported from clients/atlas-growth/2026-09-16_uk-foundation-repair-maps/prep_adjudicate.py for the
2026-09-20 AU MAPS run. Same design: tiers A, B and C in FULL plus a SEEDED 100-row random sample of
tier D (a false-negative check on the keyword tiers, not a list to buy); batches homogeneous by tier,
numbered in tier order, in adjudicate/in/batch-NNN.jsonl; site text joined through `rep_place_id`.
Only the services-page regex and the seed changed.

Usage:  python3 prep_adjudicate.py            # A,B,C full + 100 of D (seed 20260920)
...
SAMPLE_TIER, SAMPLE_N, SEED = 'D', 100, 20260920
B = 25
...
def slice_for(rec, cap=2500):
    ...
    svc = [p for p in pages if re.search(r"serv|underpin|restump|reblock|relevel|level|subsid|structur|foundation|slab|pier|piling|stump|rais|what-we-do", (p.get('label', '') + ' ' + p.get('url', '')), re.I)]
