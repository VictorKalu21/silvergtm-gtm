...
`city-fallback` → `prep` → Haiku personalise → `fill` → `check`. **Config:** `personalize-config-generators.json` (3-lead test SIGNED OFF 2026-09-26; surname-prefix first names KEPT (operator); electricians -> 'generator installation', 'HVAC and generators', original US offer line unchanged; force-added to git on operator OK). **7-lead test run 2026-09-26** (whole-home, backup-power, solar, standby dealer, res+com electrician, no-site-text, blank-city): 0 flags, 0 fallbacks, blank city read from the site; SIGNED OFF 2026-09-26 (Abtech-style 'Electrical … #1 Generator Dealer' stays 'generator installation'). **Full fill of tranche 01 started 2026-09-26:** `city-fallback` → `owner/city_overrides.json` (39 of 40 blank cities: 20 geocoded town/city/village, 19 county; 1 left for the reader) → `base --city-overrides` → `prep --batch 40` = batches 0–56 (2,266 leads with site text) + job-side batches 57–67 for the 422 no-site-text leads (prep skips them; they are read from name + Maps types so an electrician never gets the 'standby generators' fallback) → workflow `personalize-batches` (one Haiku per batch) → `fill` → `redo` loop → `check`. **DONE 2026-09-26: `owner/plusvibe_upload_t01.csv` — 2,683 rows, 1,955 named (name_basis local_match), 0 flags, 0 redo, 0 blank city, `check` clean. NOT UPLOADED.** Mix: generator installation 1,414 · standby generators 710 · HVAC and generators 336 · solar and backup power 98 · home generators 78 · whole-home 45 · home backup power 7; visit wording estimates 2,159 · quotes 490 · assessments 39. Held in `owner/plusvibe_held_t01.csv` (5): Elite Crete Systems (concrete coatings, not ICP) + 4 duplicate-email branch rows (one row per address kept, named first). 1 fallback-only row (Greentech Fuel Management). 19 no-text leads re-read after a TLS-tolerant fetch (`owner-uak/`, batch 68 overrides 57–67). R.G. Behers city = Bridgeville (site footer, added to `city_overrides.json`) of the tranche-01 base (`owner/plusvibe_base.csv`, 2,688 rows, 40 blank city). Was: the generator
