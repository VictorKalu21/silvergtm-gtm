...
| | |
|---|---|
| Current run | `2026-09-20_au-foundation-repair-maps` — **GATE 1 READY FOR THE OPERATOR, scrape not started** (2026-09-20). Keys received and written to the gitignored env (scraper.tech, Firecrawl 143 credits, Supabase URL + service role — REST/Storage probed OK, schema not yet applied). **3-call Maps probe run**: 305 rows (225 AU / 80 US), restumping densest, `foundation repair` 53% US pins; dry run through the engine → 147 of 225 AU rows kept after 3 probe-driven config changes (engineer types off the primary deny, bare geotech off the name deny, no floor in the generic recovery) + a proposed low-rated recovery. Numbers in GATE1.md. Scoping files written and tracked: `ICP-au.md`, `atlas-growth-au-config.json` (+ `recover-generic-au-config.json`, `recover-unrated-au-config.json`), `gen-runsheet-au.js` → `atlas-growth-au-runsheet.csv` (182 tiles × 10 queries = 1,820 rows, 8 shards, generator guards passed), `owner-prompts/au-foundation-repair.md` (draft, GATE 6), `personalize-au-config.json` (draft, 3/7-lead tests), run folder with `.skill-check`, `GATE1.md` (draft — Maps probe pending), `PIPELINE.md`, `RUN-NOTES.md` (registry probes pasted). **Blocked on the operator:** GATE 1 decisions (see GATE1.md tail), MV/BB keys, Supabase MCP auth + schema, store.js approval (client, whole-of-Australia vs east coast + Perth/Adelaide, restumping/reblocking as ICP, review floor, salutation fallback, Supabase-first, Maps spend). Supabase `store.js` PROPOSED in `skills/google-maps-scrape/IMPROVEMENTS.md` (OPEN 2026-09-20) with `store/schema.sql`; needs approval + test before it is built. Registry probes 2026-09-20: **NSW verify.licence.nsw.gov.au is a working keyless owner registry** (POST advQuery, pageSize ≤ 10; 122 current Underpinning-and-Piering contractor licences; detail records name the Director and Nominated supervisor — grammar in RUN-NOTES.md), VBA BAMS Salesforce community reachable, QBCC refused at the proxy, SA behind reCAPTCHA, WA publishes register PDFs + an Angular search app |
