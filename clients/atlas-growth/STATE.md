...
| | |
|---|---|
| Current run | `2026-09-20_au-foundation-repair-maps` — **SCRAPE RUNNING** (launched 2026-09-20 after GATE 1 approval "yes to all, your recs": whole of Australia, restumping/reblocking ICP, floor 5 + low-rated recovery, keep `foundation repair`, ~4,750 Maps calls, bare "Hi,", Supabase store first). 8 shards × 227 tiles, `run-scrape.js --resume`, ~65% at the last check, no errors in any log. **Shipped while it runs:** `store.js` + `store-sync.js` + 18-check test (engine, DONE; schema `store/schema.sql` still to be pasted by the operator into the Supabase SQL editor — every store call exits 2 until then, files stay the source of truth); run scripts ported to AU (merge-shards, stageB_classify, prep_adjudicate, merge_adjudication, adjudicate/PROMPT.md, gate3-stats, blank_centroid); **`registry_nsw.py`** (keyless NSW licence API — one-token search, zero-based pages, Director + Nominated supervisor per licence; 3-lead probe passed) and **`registry_wa.py`** (WA builder register PDF, 6,456 contractors with nominated supervisors; probe passed); registry profile in the planner library. **Next:** shards finish → `merge-shards.js` → qualify ×4 → `blank_centroid.py` → footprint gate → GATE 3 audit to the operator (incl. the empty-`google_types` name-only pass) → store push. Blocked on the operator only for: schema paste, MV/BB keys (later), Firecrawl residue go (later). Supabase MCP was added and removed the same day (operator: keep it simple) |
