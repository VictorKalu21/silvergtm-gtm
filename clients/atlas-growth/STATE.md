...
| | |
|---|---|
| **New vertical (opened 2026-09-24)** | **US residential standby-generator installers** — ICP in `ICP-generators.md`. Sources deep-dived (OEM dealer locators = spine, Maps = gap-fill, open permit data = proven-installer signal); run folder `2026-09-24_us-generator-dealers/` — Briggs (2,444 US) + Champion (476 US) pulled and normalised to the engine lead shape (`raw/leads_<brand>.csv`, gitignored); Generac / Kohler / Cummins pulls in progress via web-scrape-triage. $0 spent. Maps leg built, NOT run: 477 anchors × 5 queries = 2,385 runsheet rows (`atlas-growth-generators-runsheet.csv`), config + owner prompt written; **blocked on `SCRAPER_TECH_KEY`** (not in the cloud container). **Every row runs the `google-maps-scrape` pipeline from STEP 5b down — owner-finding and the Plusvibe upload are mandatory** (operator, 2026-09-24). Open: plumber rule, size floor, go on the Generac zip-grid pull. |
