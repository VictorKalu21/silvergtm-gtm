...
| Email waterfall (`email-waterfall` skill) | **100-contact test: QuickEnrich + TryKitt free tier done, AI Ark pending** (trial key quota-blocked; retry scheduled 21:57Z). TryKitt free tier: 0 of 58 found, throttled 418 on 16; usable only when funded | QuickEnrich: 21 real emails / 100, 12 sendable after MillionVerifier, 7 catch-all kept as risky (BounceBan off by operator decision), 8 empty records free. 21 QE + 22 MV credits spent. 796 named leads still need an email |
| `clay.csv` | **not built** | — |
