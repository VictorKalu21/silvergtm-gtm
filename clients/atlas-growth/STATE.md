...
| | |
|---|---|
| **New vertical (opened 2026-09-24)** | **US residential standby-generator installers** — ICP `ICP-generators.md`, run `2026-09-24_us-generator-dealers/`. OEM dealer lists (Generac 13,171 · Briggs 2,444 · Cummins 2,415 via Wayback · Kohler 1,626 · Champion 476 → 17,795 companies → 16,942 after RV/marine/big-box deny) + nationwide Maps (5,041 calls, 28,524 unique → 6,870 qualified) → **`leads_combined.csv` 21,680 leads**, 13,639 root domains + 6,961 no-website. **Site-text fetch running.** Next: name-to-domain on the no-website track → STEP 5e fit classify → owner-finding → emails → Plusvibe. **Cross-run dedupe WAIVED for this vertical (operator, 2026-09-24):** `build-netnew` found 0 prior Atlas feeds in the cloud container (foundation-run CSVs are gitignored, on the operator machine / Supabase); foundation contractors and generator dealers are different trades, so owner-finding proceeds without that history. Rotate the scraper.tech key (pasted in chat 2026-09-24). |
