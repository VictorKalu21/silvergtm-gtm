...
for (const c of companies) { const d = domains[`${c.ats}:${c.slug}`]?.domain; if (!d) continue; const v = verdicts[`${c.ats}:${c.slug}`]; if (v && v.keep === false) continue; if (c.status !== "pass" && !(v && v.keep === true)) continue; byDomain.set(d, { ...c, domain: d, verdict: v || {} }); }
...
for (const p of people) {
  ...
  leads.push({ name: `${p[P.first] || ""} ${p[P.last] || ""}`.trim(), company: c.company, role: title, source_link: c.best_url, linkedin_url: p[P.linkedin] || "", fit_note,
    evidence: c.evidence, posting_title: c.best_title, posting_date: c.best_posted_at, ats: c.ats, domain: d, company_vertical: c.verdict.vertical || "", company_size: c.verdict.size || "", email: p[P.email] || "", run_date: runDate });
}
writeCSV(`leads_${runDate}.csv`, ["name", "company", "role", "source_link", "linkedin_url", "fit_note", "evidence", "posting_title", "posting_date", "ats", "domain", "company_vertical", "company_size", "email", "run_date"], leads);
