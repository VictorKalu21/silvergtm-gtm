...
w('mv', [{ key: 'jane@acme.com', result: 'ok' }, { key: 'bob@bravo.com', result: 'invalid' }, { key: 'robert@bravo.com', result: 'ok' }, { key: 'dan@delta.com', result: 'catch_all' }, { key: 'fay@fox.com', result: 'invalid' }, { key: 'fay.fox@fox.com', result: 'ok' }, { key: 'gus@golf.com', result: 'catch_all' }, { key: 'hank@acme.com', result: 'ok' }]);
fs.writeFileSync(path.join(tmp, 'seeds.csv'), 'email,first_name,last_name\njane@acme.com,Jane,Acme\n');
const rep = JSON.parse(execFileSync('node', [S, '--dry-run', '--rungs', 'quickenrich,aiark,trykitt,pattern_seeded,pattern', '--seeds', path.join(tmp, 'seeds.csv')], { env: { ...process.env, IN: path.join(tmp, 'in.csv'), OUT_DIR: out }, encoding: 'utf8' }));
...
check('F pattern: first invalid, first.last ok -> sendable via pattern', byId.F[6] === 'fay.fox@fox.com' && byId.F[7] === 'pattern' && byId.F[8] === 'sendable');
check('G pattern on a catch-all domain never ships a guess', byId.G[8] === 'none' && byId.G[6] === '');
check('H seeded pattern from jane@acme.com -> hank@acme.com sendable', byId.H[6] === 'hank@acme.com' && byId.H[7] === 'pattern_seeded' && byId.H[8] === 'sendable');
check('I no seed at domain -> blind pattern only (none here)', byId.I[8] === 'none');
check('report per rung', rep.sendable === 4 && rep.risky === 1 && rep.none === 4 && rep.rungs.quickenrich.found === 3 && rep.rungs.quickenrich.sendable === 1 && rep.rungs.aiark.sendable === 1 && rep.rungs.trykitt.no_credits === 1);
