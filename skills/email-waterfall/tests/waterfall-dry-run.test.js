...
w('mv', [{ key: 'jane@acme.com', result: 'ok' }, { key: 'bob@bravo.com', result: 'invalid' }, { key: 'robert@bravo.com', result: 'ok' }, { key: 'dan@delta.com', result: 'catch_all' }, { key: 'fay@fox.com', result: 'invalid' }, { key: 'fay.fox@fox.com', result: 'ok' }, { key: 'gus@golf.com', result: 'catch_all' }]);
const rep = JSON.parse(execFileSync('node', [S, '--dry-run', '--rungs', 'quickenrich,aiark,trykitt,pattern'], { env: { ...process.env, IN: path.join(tmp, 'in.csv'), OUT_DIR: out }, encoding: 'utf8' }));
...
check('report per rung', rep.sendable === 3 && rep.risky === 1 && rep.none === 3 && rep.rungs.quickenrich.found === 3 && rep.rungs.quickenrich.sendable === 1 && rep.rungs.aiark.sendable === 1 && rep.rungs.trykitt.no_credits === 1);
