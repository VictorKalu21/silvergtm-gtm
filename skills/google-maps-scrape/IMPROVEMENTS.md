...
## DONE 2026-09-20 (engine, cross-run memory): `store.js` + `store-sync.js` — a Supabase store so fetches, verdicts and registry matches are a QUERY on the next run, not a re-buy

**Status:** DONE 2026-09-20 (operator approval "yes to all, your recs" + "keep it simple") — shipped `store.js` (module), `store-sync.js` (CLI) and `tests/store.test.js` (18 checks, fake PostgREST, no network). **Design simplified from the proposal below on the operator's word:** no `--store` flag inside six engine scripts; ONE module + ONE CLI that the pipeline calls between steps, and reuse happens by seeding a script's input from a pull. Every existing engine script and test is untouched. Schema `store/schema.sql` is applied ONCE by the operator in the Supabase SQL editor (PostgREST cannot run DDL; the MCP route was dropped as bloat). Until it is applied every write returns null with one WARN and `store-sync.js` exits 2; files stay the source of truth and the shard CSVs are backfilled afterwards (`store-sync.js places`). Live probe 2026-09-20 with the service key: REST 200 / Storage 200 / `places` 404 PGRST205 (schema pending). The `--store`-inside-the-scripts design below is kept as the proposal record; if a future run wants it, it is additive.
**Shipped surface.** `store.open()` → null without keys; `upsertPlaces` (chunked, `first_seen_run` preserved via a select-then-split), `upsertSiteText` (dedupe per root_domain, ok wins), `upsertVerdicts` (unverified never written), `upsertRegistry`, `upsertContacts`, `run`/`finishRun`/`ledger`, `pullSiteText(domains, maxAgeDays=180)`, `pullVerdicts(emails, maxAgeDays=90)`, `pullPlaces(country)`. CLI sub-commands: `run finish places site-text verdicts registry contacts ledger pull-site-text pull-verdicts pull-places`. Storage (raw shard tarballs) NOT built — the shard CSVs in `places` make a re-scrape unnecessary, which was the point.
...
## MEDIUM (fetch rungs): no TLS-tolerant rung — sites with a broken certificate are written off, and a 12-way parallel curl through the egress relay reads as "all dead"

**Status:** OPEN · found 2026-09-26 (Atlas Growth US generator run, the 422 tranche-01 leads with no site text). 281 of them had a website Scrapling had never tried. A Scrapling probe on 3 got 0/3 (one no-DNS, one Cloudflare 525, one timeout). Classifying the failures with curl's `%{errormsg}` showed they are mostly **broken sites, not bot walls**: 60 `CONNECT tunnel failed, response 502` (dead/unreachable origin; 13 of 40 sampled had no DNS), 41 connection reset, and **~70 TLS failures** (26 certificate name mismatch, 10 self-signed, 7 expired, 6 missing issuer, 20 handshake alerts). A fetch that ignores certificate errors (`curl -k -L`, Chrome UA, 4 in parallel) returned 200 on 53 of 281; **19 were the business's real site** after a guard (text ≥ 400 chars, a distinctive business-name token present, a trade word present, not "coming soon"/parked). The guard mattered: the other 34 were parked domains, "coming soon", a DreamHost "site not found", JS "Loading…" shells, an ADA consultant's page and a yacht club — the domain had lapsed to someone else.
