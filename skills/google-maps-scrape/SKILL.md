...
2c. **Web-search sweep for the leads 2b left unnamed** (the registry tier in `owner-finding.md`): `node prep-sweep-batches.js --leads <out>/leads_icp.csv --out <out>/owner/sweep2 --have <out>/owner/contacts_read.jsonl --batch 20 [--limit N]` queues only still-unnamed leads with a ready `"<business>" <city> <ST> owner` query and the vertical's registry domain (default `bbb.org`). One Haiku subagent per batch with `owner-sweep-subagent.md` (at most two `WebSearch` calls per lead, branch-mismatch rejection, verbatim snippet evidence), then `node merge-owner-reads.js --dir <out>/owner/sweep2 --out <out>/owner/contacts_sweep.jsonl`. **Run a first tranche (`--limit 200`) and read its cost per named lead before queuing the rest.**
