...
| 5 | session-model search in the main context | ~1.1k main-context tokens + turns | 81% | highest |
| 6 | Tier-3 fetch (web-scrape-triage) for pages that 403 | paid per page | brand-flagged roll-ups only | — |
| 7 | paid vendor SERP | two plans bought | 63 names / 753 leads, title-only | never again |

**Yield gate on rung 2:** read three batches first. If the site-named rate is under ~30%, restrict the rest of the read to leads that have a team page or prior evidence and send everything else straight to rung 3 (on Atlas Growth that saves ~27% of tokens for the same names). Rung 5 is for the high-value head and gap-fill, never bulk.
...
The deliverable is **`<out>/owner/contacts_final.{jsonl,csv}`** (`combine-owner-contacts.js` over the rung-2 read and every sweep tranche, first file wins) plus **`<out>/owner/leads_unnamed.csv`** (phone + generic mailbox only) and the verified-email file from `email-verify-debounce-bounceban`. Every contact carries a verbatim evidence quote, a role bucket, and the file it came from; every drop is counted in the merge reports. Then the email step: tier the on-site emails, verify through the two-stage gate, and run the enrichment waterfall (QuickEnrich → TryKitt, one lookup per root domain) for named leads without an address. `clay.csv` is built only if the operator asks for the Clay column; when it is, the pipeline still does not come back from Clay.
