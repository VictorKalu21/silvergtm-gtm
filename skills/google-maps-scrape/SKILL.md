...
1. `no_website_leads.csv` = rows of `leads_clean_qualified_infootprint.csv` with an empty `website`. Dedupe it too: `node build-netnew.js --new <out>/no_website_leads.csv --client <client-dir> --out <out>/no_website_netnew.csv` (place_id dedupe keeps no-website leads).
2. **Auto-run** `node search-owner.js --leads <out>/no_website_netnew.csv --config <client>-config.json --out <out>/owner_nowebsite` (finds the decision-maker via SERP without needing a domain). **Tell the operator to run their website-finder in parallel** — that's their step.
