...
Turn an ICP + a geographic footprint into a clean, deduped, footprint-filtered, **rule-qualified** business list with named decision-makers, verified emails and a sequencer upload. The hard part is **completeness per category**, **getting the geography right**, and **qualifying the universe down to the ICP** — so geography + qualification rules are settled with the user first, before any matrix is built.
...
Google Maps search is viewport-bound: one search = one `lat/lng + zoom`, capped at ~100 prominent results **per call** — but `offset` pagination WORKS (re-tested 2026-09-13) and is opt-in via `scrape_tuning.paginate`, so that ~100 is a per-CALL cap, not a per-viewport one. **Completeness comes from tiling tighter zooms across the footprint and deduping on `place_id`, not from one big search.** See `runbook.md` for the full API mechanics.

The scrape builds the *universe + geo + activity signals*. Owner name, email, employee count, and founding date are NOT on Google Maps — those come from owner-finding (STEP 6), the email waterfall (STEP 6e) and, for the sequencer, STEP 7b.
...
This skill covers scrape → qualify → owner-finding → emails → sequencer upload. It does **not** cover everything a
...
- **`<client>-config.json`** — three blocks: **`geo`** (country + footprint + region), **`qualify_rules`** (the ICP filter — STEP 5b), **`owner_query`** (STEP 6 cascade). Schema:
  ...
  ```
  **`scrape_tuning`** (optional): `{ saturation, max_depth, quad_offset, limit, paginate, max_pages }`. **`paginate: true`** pages each viewport via `offset` until exhausted — `offset` pagination WORKS (re-tested 2026-09-13; the old "it's broken" note was stale) and `scrape.js` never used it, so it was truncating the long tail on every dense tile: in the Altivox pilot **158 of 160 searches needed more than one page**, and one viewport yielded 348 unique against the ~100 the runbook called a hard cap. Default OFF so existing runs are unchanged. Two knock-on effects when it is on: `SATURATION` goes inert (a split would need `max_pages × limit` records), and `run_log.per_cell` becomes ONE roll-up event per runsheet row whose status is `ok` only if every page and every sub-tile succeeded — required, because `run-scrape.js::failedTiles` marks a tile healthy if ANY event with its `query|lat|lng` key is ok and paginated pages share that key exactly.
...
- Keepers stay in the dated run folder: `leads_clean*.csv`, `excluded.csv`, `coverage_report.json`, `run_log.json`, `owner/`, `owner-prompt.md`, the runsheet, the config.
...
Rules tagged `stage:"enrichment"` (or referencing a non-Maps field like `employee_count`) are skipped here — they're `enrich_rules`, applied after enrichment if the client ever adds one. Deterministic, $0, no re-scrape. Outputs `leads_clean_qualified.csv` (the real list) + `excluded_officp.csv` (with `drop_reason`, so the drops are auditable). Spot-check the drop reasons; tune rules with the client.
...
`--client` auto-discovers prior **shipped feeds only** — filenames matching `/^clay.*\.csv$|_netnew\.csv$/i` (`build-netnew.js:54`), skipping this run's own folder + `_archive`. It does NOT scan every `*.csv` with a `place_id` column, and intermediates (`leads_clean*`, `*_qualified*`, `excluded*`) are deliberately ignored. **Two consequences worth internalising before you rely on this:** (a) the delta output IS the memory, so anything you filter OUT of a shipped feed is also forgotten and returns next run looking net-new; (b) the secondary key is the **website host**, so every branch of a multi-site operator (all sharing one domain) is dropped as "already contacted" — fatal for any per-premises trigger. For repeat-scrape/delta jobs use `union-passes.js` instead, which keeps a pre-qualify union file as memory and dedupes on `place_id` only (see `processes/05-new-premises-delta.md`). Key = `place_id` (stable across runs); website host is a secondary key. **The client folder's prior CSVs ARE the dedupe memory.** When `build-netnew.js` runs, it prints `ref files used: N | prior place_ids: M | new: …` — if this client has been run before but N = 0 (fresh machine, moved folder, wrong root), **STOP — do not proceed to owner-finding.** A missing history means re-contacting leads already sitting in live campaigns. Recover the client folder from whoever ran it last; only treat `ref files used: 0` as valid for a genuinely brand-new client. **No-website leads are KEPT** (they can't have been captured-with-a-website before — they feed the no-website recovery track: user runs a website-finder while `search-owner.js` finds the decision-maker; a domain + a contact = a recovered net-new email lead). **`leads_netnew.csv` is the deduped list; STEP 5c-dom splits it into the owner-finding spend file and the recovery track before anything is paid for.** Dedupe is per-CLIENT — never global, never skipped.
...
- **`leads_annotated.csv`** — ALL rows + `website_class, root_domain, location_count, is_multi_location, brand_family, rep_place_id` → the `--leads` spine for the owner read, so every branch still ships as its own row. Row count never changes; only the spend collapses.
- **`domain_siblings.json`** — sibling → representative map; the owner read gives each fanned branch its representative's text (`fanned_from` records provenance).
...
3. When domains come back, `node fetch-sites.js` on the recovered domains → merge into `site_text.jsonl` for the owner read.

The with-website and recovery tracks ship **together** — recovery is MANDATORY/auto, not optional. **Two gotchas:** (a) if the client narrows scope mid-run (e.g. "drop insurance"), filter BOTH tracks, not just the primary; (b) no-website leads were extracted at the website rule so they NEVER saw the chain/deny qualify rules — **re-run the chain/deny qualify on the MERGED final list before the owner read**, or a blocklisted chain (e.g. Wells Fargo Advisors) rides in through the recovery track.

## STEP 6 — Phase 2: Owner-finding

Run the scripted owner-finder — it gets the owner name + verbatim proof for ~$0 (Haiku on pre-scraped text). **84% hit rate on the LNP ICP-1 pilot, 0 fabrications.** Full guide in **`owner-finding.md`**.
...
**Nothing under `<run>/owner/` is touched until `<run>/owner-prompt.md` exists and passes the per-vertical checklist.** The prompt is built **per vertical, every time** — it is never a static or copied file. The PreToolUse hook (`.claude/hooks/guard.py`) enforces this deterministically.
...
2. **One shape, every country** (template's "ONE SHAPE"): a SINGLE reader prompt that reads ALL available sources in one pass → one deduped `contacts` array. Site text + owner page + SERP + web-search evidence always, plus Companies House directors (AUTHORITATIVE) when `companies_house.jsonl` exists for the job. No 2-prompt variant.
...
6. Save it to **`<client>/<subscrape-slug>/owner-prompt.md`** (the job folder, never the skill folder) — that is where the hook looks for it.

Flow — **owner-finding runs in-session; there is no Clay rung** (operator decision 2026-09-12). Order the rungs cheapest-first and stop when the yield says so:

| rung | what | cost per lead (measured, Atlas Growth) | yield | precision |
|---|---|---|---|---|
| 1 | aggregate what is on disk: site text, team page, SERP text, prior evidence | ~0 | — | — |
| 2 | ONE Haiku read over the aggregate with `owner-prompt.md` (the adjudication step) | ~2.4k tokens | 24.5% home-services · ~84% healthcare | 99% grounded |
| 3 | Haiku web-search sweep, registry-restricted, still-unnamed leads only | ~4.4k tokens, 1.3 searches | ~70% | 93%, 0/30 contradicted |
| 4 | second-opinion verify on medium/low-confidence rows + a 30-row sample | ~5k tokens on ~10% of rows | — | lifts 3 toward 2 |
| 5 | Tier-3 fetch (web-scrape-triage) for pages that 403 | paid per page | brand-flagged roll-ups only | — |
| — | session-model search | — | — | not a rung: every lead is treated the same, so the session model does all or none, and "all" is the most expensive way to get names the cheap rungs already get right 93–99% of the time |
| — | paid vendor SERP | two plans bought | 63 names / 753 leads, title-only | never again |

**FACT (UK trades, measured on Atlas Growth 2026-09-16 UK MAPS run, 860 leads):** Companies House is the PRIMARY owner registry for a UK trade vertical — **504 of the 586 names on that run came from it** (web search/SERP 68, site 12, email local part 2), so the registry rung runs before the sweep, not after it. A CH match accepted on the **town/city name alone is a CANDIDATE, not an answer** (~21% wrong on that run) — the reader must judge it with the prompt's registry rule, exactly as it judges a `low_confidence` one.

**Yield gate on rung 2:** read three batches first. If the site-named rate is under ~30%, restrict the rest of the read to leads that have a team page or prior evidence and send everything else straight to rung 3 (on Atlas Growth that saves ~27% of tokens for the same names). Rung 4 runs on EVERY shaky row, not a sample: all leads matter equally (operator, 2026-09-12).

**Then STEP 6e — emails, the `email-waterfall` skill** (operator decision 2026-09-13: the waterfall is part of owner-finding, not a separate hand-off). Input = every named contact at an eligible domain (`contacts_final` → `full_name, first_name, last_name, root_domain, business_name, …`). Rungs in order: state licence-board email where the state publishes one (LA, AR — see `icp-source-planner/library/state-contractor-licence-boards--home-services-emails.md`) → on-site addresses tiered and verified → QuickEnrich → `pattern_seeded` (learn the pattern from a verified address at the same domain; **no blind guessing**) → paid finders only as 50-contact probes fed with the misses → verified company mailbox as the fallback. MillionVerifier + BounceBan on everything. Measured on Atlas Growth: 21% of ICP leads end with a sendable address, 16% with a personal one; QuickEnrich ≈ 1.06 credits per sendable.

1. `node fetch-sites.js --in <out>/leads_domains.csv --out <out>/owner --concurrency 12` → `site_text.jsonl` (homepage + L2 pages incl. "Our Staff / Meet the Team"). Input is the STEP 5c-dom representatives file — one fetch per root domain; shared-host rows are skipped with a warning if one slips in. The on-site email harvest reads the **raw** HTML — `mailto:` hrefs, JSON-LD `"email"`, Cloudflare `data-cfemail` and tag-split/entity-obfuscated forms as well as the page text — and the contact page (`contact` / `contact-us` / `get-in-touch` / `enquir`) is now a default L2 target, so each record carries `emails` plus an `emails_by_source` map saying which rung found each address.
...
2b. **In-session read:** `node prep-owner-batches.js --leads <out>/leads_icp.csv --dir <out>/owner --out <out>/owner/read --batch 40` builds one `batch-N-in.json` per 40 leads from the text already on disk (site text with people pages first, dedicated owner page, SERP snippets, earlier web-search evidence). Dispatch **one Haiku subagent per batch, all in one message**, with the prompt in `owner-read-subagent.md` (or run the saved workflow `owner-read` with `args: {run: <abs run dir>, batches: N}` from `.claude/workflows/`): it reads `owner-prompt.md` and writes `batch-N-out.json`. The harness runs at most min(16, CPUs−2) agents at once, so on a 4-CPU box 23 batches take ~100 min; plan for it. Then `node merge-owner-reads.js --dir <out>/owner/read` → `<out>/owner/contacts_read.jsonl` + `.csv` + `read_report.json` (drops counted: evidence without the name, bad bucket, role word in a name). **The model is the only reader. A regex or keyword parser never names a person** (Atlas Growth banked 24 business names as people that way). Cost is ~1 Haiku read per lead on pre-scraped text; it does not search.
2c. **Web-search sweep for the leads 2b left unnamed** (the registry tier in `owner-finding.md`): `node prep-sweep-batches.js --leads <out>/leads_icp.csv --out <out>/owner/sweep2 --have <out>/owner/contacts_read.jsonl --batch 20 [--limit N]` queues only still-unnamed leads with a ready `"<business>" <city> <ST> owner` query and the vertical's registry domain (default `bbb.org`). One Haiku subagent per batch with `owner-sweep-subagent.md` (saved workflow `owner-sweep`, same args shape; at most two `WebSearch` calls per lead, branch-mismatch rejection, verbatim snippet evidence), then `node merge-owner-reads.js --dir <out>/owner/sweep2 --out <out>/owner/contacts_sweep.jsonl`. **Run a first tranche (`--limit 200`) and read its cost per named lead before queuing the rest.** **Measured sweep yield, UK trades (Atlas Growth 2026-09-16 UK MAPS run):** with `--registry linkedin.com`, 17 batches over 331 still-unnamed leads spent ~470 WebSearch calls and named **70 (21%)** — tranche yields 23/100 · 22/100 · 24/100 · 5/31, flat across tranches. Plan the UK headline around the registry rung, not the sweep; US home services measured ~70% on this same rung. A workflow run has a **200-WebSearch budget** (`CLAUDE_CODE_MAX_WEB_SEARCHES_PER_SESSION`); agents past it return empty batches without error, and the budget is shared by runs executing at the same time, so launch the sweep as ONE run of at most 6 batches at a time (`args.batchIds`), the next when the previous reports; never one big run, never several in parallel.
...
4. `fill` — cleans each value, reports two consistency flags by place_id (`flag_visit_repeats_trade`: the visit contains the whole trade phrase; `flag_outcome_off_trade`: the outcome noun does not belong to the trade, e.g. "waterproofing jobs" for a foundation-repair shop; `flag_word_used_3x`: one word in all three trade values, e.g. "concrete leveling / leveling estimates / leveling jobs"; `flag_free_in_visit`), which are redone as a higher-numbered batch whose values override (prompt in `personalize-subagent.md`), (quotes, trailing period, too long, "cannot" → fallback), fills the template, and **refuses to write** if any row breaks the name rule. `check --csv` re-applies the rule and the unfilled-placeholder check to any CSV, so a hand edit or a merge is caught before upload.
...
- Corporate exclusion by name blocklist is the cheap ~80%; the final chain/independent call is the site-text classifier (STEP 5e) + the human builder.
