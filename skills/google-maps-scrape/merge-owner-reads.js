...
const rep = { batches: ins.length, batches_missing_out: [], leads: 0, leads_with_contacts: 0, contacts: 0, owner_level: 0,
  dropped_no_evidence_name: 0, dropped_bad_bucket: 0, dropped_roleword_name: 0, dropped_unknown_place_id: 0, by_source: {} };
const lines = [];
for (const f of ins) {
  ...
  for (const [pid, v] of Object.entries(out)) {
    ...
    if (kept.length) rep.leads_with_contacts++;
    rep.contacts += kept.length; rep.owner_level += kept.filter(c => c.role_bucket === 'owner_or_partner').length;
    const primary = kept.find(c => c.role_bucket === 'owner_or_partner') || kept.find(c => c.role_bucket === 'gm') || kept.find(c => c.role_bucket === 'marketing') || kept.find(c => c.role_bucket === 'office_manager') || kept[0];
    lines.push(JSON.stringify({ place_id: pid, business_name: lead.business_name, city: lead.city, state: lead.state, brand_family: lead.brand_family,
      ...
      confidence: v.confidence || '', needs_review: kept.length === 0, source: 'model_read' }));
  }
}
