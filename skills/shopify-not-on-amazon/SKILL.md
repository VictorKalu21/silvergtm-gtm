...
- **Bad search term = bad verdict.** The term comes from `meta.json name` / the `<title>`; "top" (kirby.com), "new car" (vinylfrog.com), "urinary tract health supplements" (uqora.com) all searched the wrong thing. Every row that reaches the deliverable gets its search term eyeballed (`Amazon search term used` column); fix the bad ones in `{RUN}_query_overrides.json` and re-run `ONLY=` on them. An override is the **brand name only** ("goli", "fox factory", "ag adriano goldschmied" = the name Amazon uses), never name + category ("goli gummies" made the matcher demand both words and lost the Goli store). `dataforseo.mjs` reads the same overrides.
- **Tablet product pages use single-quoted attributes** (`id='productTitle'`, `id='sellerProfileTriggerId'`); a double-quote-only regex silently returns an empty title/seller. The title still comes back empty on most tablet pages (open item in IMPROVEMENTS), so the title-based byline rules rarely fire; the domain-based rules carry the load.
- **After any matcher change run `RESCORE=2`** on every run directory: it re-judges the stored product pages in seconds and costs no Amazon requests (`RESCORE=1` = upgrade-only). Read every line it prints: a `brand_store -> none` flip on a garbage search term ("best" for goli.com, "fox" for ridefox.com) means the old verdict was a coincidence, not that the brand is off Amazon; put the real name in `{RUN}_query_overrides.json` and re-run `ONLY=` on those.
- **Single-word brand names are the weak spot.** "Force" matched the Force Factor store, "Berkley" matched Berkley Jensen, "Pura Vida" matched Pura Vida Moringa. A store/seller name that carries a distinctive word the brand name lacks is a different entity (`noExtraWords`), unless the store name starts with the whole domain root ("Darn Tough Vermont" for darntough.com). Sold by Zappos / Shopbop / Woot = sold by Amazon (first-party).
