...
async function enrich(r) {
  const d = r.domain; const emails = new Map(), phones = new Map(), people = new Map(); let linkedin = r.linkedin || null; const foreign = new Set();
  const FOREIGN = { uk: /\+44[\s\d]{8,}|\bUnited Kingdom\b|\bLtd\b(?![^.]{0,30}\bUSA?\b)/, au: /\+61[\s\d]{8,}|\bPty\.? Ltd\b|\bAustralia\b|\.com\.au\b/, ca: /\bCanada\b|\bOntario\b|\bBritish Columbia\b|\.ca\b(?=[\s"<])/, de: /\+49[\s\d]{8,}|\bGmbH\b|\bGermany\b/, it: /\+39[\s\d]{8,}|\bS\.?r\.?l\.?\b|\bItaly\b/, fr: /\+33[\s\d]{8,}|\bSAS\b|\bFrance\b/, nl: /\bB\.V\.|\bNetherlands\b/, nz: /\bNew Zealand\b|\.co\.nz\b/ };
  for (const p of PATHS) {
    ...
    if (/policies|contact|about/.test(p)) for (const [k, re] of Object.entries(FOREIGN)) if (re.test(t)) foreign.add(k);
  ...
  return { domain: d, emails: ranked.slice(0, 5), personalEmail: ranked.find((e) => !GENERIC_LOCAL.test(e)) || null, phones: [...phones.keys()].slice(0, 3), linkedin, people: [...people.entries()].sort((a, b) => b[1] - a[1]).slice(0, 3).map(([n]) => n), foreignParentHints: [...foreign], pagesHit: [...new Set([...emails.values(), ...phones.values()])] };
}
let i = 0, n = 0;
