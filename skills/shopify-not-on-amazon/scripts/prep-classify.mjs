...
// incremental: survivors that already have a verdict (or sit in a pending batch) are skipped; new batches get fresh indexes
const have = new Set(); let b = 0;
for (; existsSync(`${DIR}/${RUN}_review_batch_${b}_out.json`) || existsSync(`${DIR}/${RUN}_review_batch_${b}.json`); b++) {
  for (const f of [`${DIR}/${RUN}_review_batch_${b}_out.json`, `${DIR}/${RUN}_review_batch_${b}.json`]) if (existsSync(f)) for (const v of JSON.parse(readFileSync(f, 'utf8'))) if (v && v.domain) have.add(v.domain.toLowerCase());
}
const start = b;
const survivors = rows.filter((r) => r.status === 'pass_free_gates' && !have.has(r.domain.toLowerCase())).sort((a, b) => (a.rank || 9e9) - (b.rank || 9e9))
...
console.error(`wrote ${b - start} batch file(s) (${start}..${b - 1}) for ${survivors.length} unclassified survivors (BATCH=${BATCH}); ${have.size} already classified or pending.`);
