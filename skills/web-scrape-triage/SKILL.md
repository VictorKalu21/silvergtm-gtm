...
- **If it's not in source, capture it with a headless-Chrome netlog** — render the page and read every request it fired:
  ...
  - **When netlog can't reach the endpoint (SPA needs interaction, or an anti-bot challenge sits in front), Scrapling captures the response *bodies* instead** — `StealthyFetcher`/`DynamicFetcher` grab every background XHR/fetch during one render. The combine: pay the render **once** to get *both* the replayable request recipe (params/headers → paginate cheaply) *and* page 1's body for free, then drop to cheap replay for pages 2…N. Unlike plain `chrome --log-net-log` it captures **behind an anti-bot challenge** (netlog-via-plain-chrome dies at the challenge). Not a replacement for netlog — it costs a full render; use it when netlog can't get there.
...
- **Firecrawl** — `POST api.firecrawl.dev/v1/scrape {url, formats:["markdown"]}`, `Authorization: Bearer fc-...`. Renders JS + clears Cloudflare; clean markdown. The managed default. ~$0.60-0.83/1k pages; `/extract` (LLM) adds ~4×.
- **Scrapling (self-hosted, free) — try this before paying Firecrawl.** `StealthyFetcher.fetch(url, solve_cloudflare=True)` (patched Chromium) has an explicit Turnstile/Interstitial solver. Tested 2026-09: it cleared a live **managed Turnstile** and pulled a full bot-managed **Crunchbase** profile (~35–40s each) — but **failed on G2's 403 (Imperva-class WAF)**. So it's a *free attempt*, not a guaranteed bypass: when it returns a block page, escalate to Firecrawl / a vendor-matched unlocker for that confirmed-hard subset. Its `Fetcher.get(url, stealthy_headers=True)` is the **curl_cffi TLS/JA3 rung in the same library** (got Clutch where a raw client 403'd). Slow — full render per solve; reserve for the confirmed-challenge subset, never bulk. Install: `pip install "scrapling[fetchers]"` + `scrapling install`.
