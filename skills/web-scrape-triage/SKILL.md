...
- **Jina search `https://s.jina.ai/?q=<query>`** — ⚠️ **no longer keyless.** As of 2026-09 it answers `401 AuthenticationRequiredError` and requires an API key. Verified in a live session; do not plan a pipeline around it being free. With a key it is still a good LLM-ready SERP.
...
- Reach for a paid SERP (serper/scraper.tech/Firecrawl) ONLY after blowing past those free tiers at real volume — most jobs never do. **Bulk name→domain at ~2,000 rows is past it:** the built-in WebSearch tool is quota-capped (see below) and Jina now needs a key, so a keyed search API in a script is the only path that finishes.
- **The built-in WebSearch tool has a per-session budget of roughly 200 calls.** It is for interactive lookups, not pipelines. Subagents share it, so fanning out 20 agents starves them all — measured on a real run: 84% fill with 2 concurrent agents, 31% with 20, with agents reporting "web search budget exhausted". Above ~2 concurrent, move to a scripted search API.
...
| `200`, ~100-byte body, `window.onload=...href="/lander"` | **parked domain** — you guessed the name wrong | **STOP treating it as a site.** Detect and reject: `/lander` redirects, sedoparking, afternic, "this domain is for sale". A guessed `<name>.com` that 200s from a parking page is the classic false positive |
| `403` with body `Host not in allowlist:` | your client bypassed the egress proxy | not the site's fault — fix the client (see the Node note above) |
...
- **WebSearch** (built-in tool) — SERP discovery + domain resolution, no key. The Tier-2 workhorse. Scriptable SERP for pipelines: **Brave Search API** (free tier) or **Firecrawl `/v2/search`** (keyed). Jina `s.jina.ai` is **no longer keyless** — it 401s without a key.
...
- **Firecrawl** — anti-bot rendered scrape (Cloudflare) + JS render. `POST api.firecrawl.dev/v1/scrape`, `Authorization: Bearer fc-...`. Key is provided per-session (not stored on disk); roll it after use. NOTE (Windows): live network calls must run via PowerShell — the Bash sandbox has no network (DNS fails).
  **NOTE (Claude Code cloud session): the failure is different and far more dangerous.** Egress works, but goes through an agent proxy, and **Node's `http`/`https` modules ignore `HTTPS_PROXY`**. They bypass it and return HTTP 403 with a body reading `Host not in allowlist: <domain>` — roughly 100 bytes that a scraper happily treats as page content. Nothing throws; you get a full run of error pages scored as if they were websites. Use global `fetch` with `NODE_USE_ENV_PROXY=1`, and make a missing flag fatal rather than trusting yourself to remember. `curl` reads the proxy correctly, so curl working is NOT evidence that your Node fetcher does.
